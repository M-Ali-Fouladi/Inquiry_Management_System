...
namespace CustomMembershipEF.Infrastructure
{
    public class CustomMembershipProvider : MembershipProvider
    {
        ...
        public override MembershipUser CreateUser(string username, string password, string email, string passwordQuestion, string passwordAnswer, bool isApproved, object providerUserKey, out MembershipCreateStatus status)
        {
            ...
            if (user == null)
            {
                var userObj = new Models.tbl_Admin { AdminUserName = username };

                using (var modelContext = new Models.Model1())
                {
                    modelContext.AddUser(userObj);
                }
        ...
        public override bool ValidateUser(string username, string password)
        {
            ...
            dirEntry = new DirectoryEntry("LDAP://172.21.3.6", username, password);
            Guid Loggedin = dirEntry.Guid;
            using (var LabEntities = new Models.Model1())
            {
                 var requiredUser = LabEntities.GetUser(username);
